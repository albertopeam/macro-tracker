...
class _DailyTabState extends ConsumerState<DailyTab>
  ...
  String get _dateKey => _date.dateKey;
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed && !_date.isToday) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: _DateNav(
          ...
          onPrev: _prevDay,
          onNext: _date.isToday ? null : _nextDay,
          onTodayTap: _date.isToday ? null : _goToday,
        ),
  ...
  Future<void> _showCopySheet(
    ...
    final targetDate = await showModalBottomSheet<String>(
      ...
      builder: (_) => _CopyMealSheet(mealType: mealType, isToday: _date.isToday),
    );
  ...
  String _fmtDateLabel(String dateKey) {
    if (dateKey == DateTime.now().dateKey) return 'Today';
    if (dateKey == DateTime.now().add(const Duration(days: 1)).dateKey) return 'Tomorrow';
