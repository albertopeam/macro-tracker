...
Date logic lives in `lib/core/extensions/date_time_x.dart` as a `DateTimeX` extension — never inline it in widgets.

- Key generation (`dateKey`, `monthKey`), day comparisons (`isToday`, `isSameDay`) all belong in the extension.
- Widgets may only call display-only `DateFormat` patterns (e.g. `DateFormat('EEE, MMM d').format(date)`) — anything producing a logic-bearing string (DB keys, provider keys) must go through the extension.
- Never use unscoped global functions for date operations — prefer extension methods (`d.isSameDay(other)` instead of the `table_calendar` global `isSameDay(a, b)`).
