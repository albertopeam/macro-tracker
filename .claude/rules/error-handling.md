...
Do not substitute fake values when data can't be parsed or converted.
A fallback of `DateTime.now()`, `0.0`, or `''` for user-logged data silently corrupts what the user sees.

**User-logged data** (meal entries, food rows from DB): let parse/cast errors propagate — the `AsyncError` UI is correct and honest.

**Preference/settings data** (goals, profile from SharedPreferences): safe defaults are acceptable because the data is user-configurable and loss is recoverable. Clear the corrupted key and log with `debugPrint`.

```dart
// ✅ preference data — safe default OK
try {
  return Foo.fromJson(jsonDecode(raw));
} catch (e) {
  debugPrint('Failed to load Foo: $e — clearing corrupted data');
  await prefs.remove(fooKey);
  return Foo.defaults();
}

// ✅ user-logged data — let it propagate
createdAt: DateTime.parse(map['created_at'] as String), // throws → AsyncError
```

## Try/catch in providers (save operations)

Persist to storage **before** updating state. If persistence fails, set `AsyncError` — never leave memory ahead of storage.

```dart
Future<void> save(Foo foo) async {
  try {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(fooKey, jsonEncode(foo.toJson()));
    state = AsyncData(foo);
  } catch (e, st) {
